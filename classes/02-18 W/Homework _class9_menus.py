...
2) explain in your own words what the program does
    ...
    The program uses a structure of nested loops and conditional statements to navigate through menu options.
    Choice is the most recent selection by the user. to_top is assumed to be false unless its triggered to go back to top.
    Other transitions are going up a level if the answer is none and going a level further if there is a valid answer.
    The invariants are that only one menu level is active at a  time, no answer moves up one level, once you reach the end
...
There are 8. For clients: View Client Summary, Manage Client Cash, New Individual, New Joint.
For portfolios: Buy, Sell, Holdings Snapshot, P/L Report
'''
'''
